...
<h3>
  <?php echo __("List of event  from provider %provider%", array('%provider%' => $provider->getFullName()))?>
</h3>
...
<?php include_partial("profile/flashes")?>
    ...
    <div class="pagination">
        <div class="page_list">
            <ul class="paging">
            	<li><a href="<?php echo url_for('event/provider?slug='.$provider->slug."&page=1")?>"><?php echo __("Inicio")?> </a></li>
            		<?php foreach ($pager->getLinks() as $page): ?>
            		<?php if ($page == $pager->getPage()): ?>
            	<li><?php echo $page ?></li>
            		<?php else: ?>
            	<li><a
            		href="<?php echo url_for('event/provider?slug='.$provider->slug."&page=".$page)?>"><?php echo $page ?></a></li>
            		<?php endif; ?>
            		<?php endforeach; ?>
            
            		<li><a
            		href="<?php echo url_for('event/provider?slug='.$provider->slug."&page=".$pager->getLastPage()) ?>"><?php echo __("Final")?>
            	 </a></li>
            </ul>        
        </div>
