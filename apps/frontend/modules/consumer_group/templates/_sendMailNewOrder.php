...
?>
<?php echo __("Se acaba de abrir un pedido para la/el %s% %y% con el nombre de %&%",array("%s%"=>$provider->ProviderType->name,"%y%"=>$provider->name,"%&%"=>$order->name))?><?php echo PHP_EOL?>
...
<?php if ($order->group_comment):?>
    <?php echo __("Comentarios al pedido: %&%",array("%&%"=>$order->group_comment))?>
<?php endif;?>